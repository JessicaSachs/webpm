...
export async function testPackageJsonResolution() {
  try {
    ...
    console.log('Package.json to resolve:', JSON.stringify(packageJson, null, 2));
    ...
    for (const result of prodResults) {
      console.log(`  ${result.root.package.name}@${result.root.package.version}: ${result.totalPackages} packages`);
    }
    ...
    const withPeersResults = await webpm.resolveAndFetchPackageJson(packageJson, {
      includeDevDependencies: false,
...
export async function testMinimalPackageJson() {
  try {
    ...
    for (const result of results) {
      console.log(`  ${result.root.package.name}@${result.root.package.version}`);
      console.log(`    Total packages: ${result.totalPackages}`);
      console.log(`    Total files: ${result.totalFiles}`);
      console.log(`    Resolution time: ${result.timings.resolutionTime.toFixed(2)}ms`);
      console.log(`    Fetching time: ${result.timings.fetchingTime.toFixed(2)}ms`);
    }
...
export async function testEmptyPackageJson() {
  try {
    ...
    console.log(`Empty package.json resolved: ${results.length} dependencies (should be 0)`);
