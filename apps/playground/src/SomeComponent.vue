...
const resultToPrint = computed(() => {
  return (
    result.value?.map((r: FetchedDependencyTree) => {
      return {
        name: r.root.package.name,
        version: r.root.package.version,
        allFetchedPackages: Array.from(r.allFetchedPackages.values()).map(
          (p: FetchedPackage) => {
            return {
...
const resolve = async () => {
  ...
  result.value = await webpm.resolveAndFetchPackageJson(
    JSON.parse(props.packageJson),
    {
